# UNDP Update Protocol - AgriCien

This file serves as a reference for the AI assistant to perform updates to the UNDP tender monitoring dashboard without altering the design or omitting categories.

## 1. Objective
Scan the global UNDP Procurement Notices portal (English) for active opportunities matching AgriCien's 114 items and update `undp.html`.
...
## 3. AI Procedure
1. Navigate to [UNDP Procurement Notices](https://procurement-notices.undp.org/).
...
3. Filter active tenders (Negotiations/Notices).
4. Extract: Ref ID, Institution/Country, Title, Deadline.
5. Update the `tenders` array in `undp.html`.
